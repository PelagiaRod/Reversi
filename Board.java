...
public class Board {
    ...
    public void makeMove(int player, int x, int y) {
        //directions is a string which contains all the possible cells this player can go
        ...
        while (splitter.hasMoreTokens()) {      //if there are still more available moves
            ...
            if (direction.equals("UP_RIGHT")) {     //fill the between cells with the player's letter
                ...
                if (numOfMoves > 0)   numOfMoves--;       //reduce the number of moves
    ...
    public String isValidMove(int letter, int x, int y) {   
        ...
        if (x<0 || y<0) return "NOT_VALID";
        if (x>7 || y>7) return "NOT_VALID";
        //check if move leads to a non empty cell
        if (gameBoard[x][y] != EMPTY) return "NOT_VALID";
        ...
        if (row == -1) directions += "";    //we are out of bounds
        ...
        if (row == 8) directions += ""; //don't add to the directions the DOWN option (there is no cell left)
            //if at the row we've reached, i.e. 5, between them there is enemy letter (in==true)
        ...
        if (--col == -1 || --row == -1) directions += "";
        else if (gameBoard[row][col] == letter && inside) {
            directions += " DOWN_LEFT";
        }

        if (directions.equals("")){
            directions = "NOT_VALID";
        }
    ...
    public ArrayList<Move> acceptableMoves(int player) {
        ...
        for (int i=0; i<gameBoard.length; i++) {
            for (int j=0; j<gameBoard.length; j++) {      //only if the specific move is not forbidden
                if (!isValidMove(player, i, j).equals("NOT_VALID")) {
